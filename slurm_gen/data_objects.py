...
class PreprocessedData:
    ...
    def __init__(self, cache_path):
        ...
        self.name = '.'.join(os.path.basename(os.path.normpath(cache_path)).split('.')[:-1])
    ...
    def size(self):
        ...
        metadata_path = os.path.join(os.path.dirname(os.path.normpath(self.path)), '.metadata')
...
class Group:
    """Represents a group of data samples, and contains references to all contained PreprocessedData."""
    def __init__(self, cache_path):
        ...
        self._iterator = None
        self._elements = {item for item in os.listdir(self.path) if not item.startswith('.')}
    ...
    def unprocessed_size(self):
        ...
        # count the number by hand
        self._unprocessed_size = len(utils.from_pickle(os.path.join(self.path, 'none.pkl'))[1])
...
class ParamSet:
    ...
    def __init__(self, cache_path):
        ...
        self.name = os.path.basename(os.path.normpath(cache_path))
        self._iterator = None
        self._elements = {item for item in os.listdir(self.path) if not item.startswith('.') and item != 'raw'}
...
class Cache:
    ...
    def __init__(self):
        ...
        self._iterator = None
        self._elements = list(sorted(item for item in os.listdir(self.path) if not item.startswith('.')))

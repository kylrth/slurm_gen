...
def _collect_raw(raw_path, raw_files, size, verbose=False):
    ...
    Instead of deleting the raw files, return the files marked for deletion. This allows
    _assign_data to create the updated cache before deletion, avoiding the risk of the
    process being killed after deletion but before writing the updated cache. Yay for
    ...
    Returns:
        ...
        (bool or tuple(str, list, list)): either False (meaning no leftovers need to be
                                          saved), or a tuple with:
                                          - the path to the raw file where leftovers
                                            should be written,
    ...
    try:
        while len(raw_X) < size:
            utils.v_print(
                verbose, "Collected {}/{} samples so far.".format(len(raw_X), size)
            )
            ...
            if len(temp_X) <= enough:
                ...
                utils.v_print(
                    verbose,
                    "Collected {} samples from raw file. Still need {} more.".format(
                ...
                utils.v_print(
                    verbose,
                    "Collected {} samples from raw file, with {} left over.".format(
        ...
        raise utils.InsufficientSamplesError(
            size - len(raw_X), os.path.dirname(raw_path)
        )
...
def _assign_data(size, target_path, verbose=False):
    ...
    For each dataset with a specific set of parameters, the unassigned samples appear in
    the 'raw' directory.
    ...
    # iterate through the files in the raw directory until enough samples are collected
    utils.v_print(verbose, "Mutex obtained for raw path.")

    raw_files = os.listdir(raw_path)
    utils.v_print(verbose, "{} raw files found.".format(len(raw_files)))

    # collect samples from raw files, and raise an error if there aren't enough samples
    ...
    utils.v_print(
        verbose,
        "Added {} samples to existing set of {} samples, for a total of {}.".format(
    ...
    if to_save:
        ...
        utils.v_print(
            verbose,
            'Saving {} leftover samples to "{}".'.format(len(temp_X_left), raw_file),
        )
...
def _load_unprocessed_dataset(size, unprocessed_path, verbose=False):
    """Load the unprocessed dataset from cache, including unassigned samples into the
    dataset if necessary.

    Args:
        size (int or NoneType): number of samples to include. If none, every sample is
                                included.
    ...
    if os.path.isfile(unprocessed_path):  # unprocessed cache exists
        ...
        if size is None:
            utils.v_print(
                verbose,
                "No size specified; returning (len(X), len(y)) == ({}, {}).".format(
                    len(X), len(y)
                ),
            )
            return X, y
        if len(X) >= size:
            utils.v_print(
                verbose,
                "Size == {size} specified; "
                "returning first {size} of (len(X), len(y)) == ({x}, {y}).".format(
                    size=size, x=len(X), y=len(y)
                ),
            )
        ...
        utils.v_print(
            verbose,
            "Insufficient data ({} < {}). Calling _assign_data.".format(len(X), size),
        )
    ...
    if size is None:
        utils.v_print(
            verbose,
            "No size specified and no unprocessed cache found. Returning empty lists.",
        )
        return [], []
...
def _apply_preproc(preproc, batch_preproc, X, y, verbose=False):
    ...
    Args:
        preproc (callable): Preprocessor function, with the call signature specified in
                            the docstring for get_data.
        batch_preproc (bool): Whether to apply the preprocessor to the entire dataset or
                              one point at a time.
...
def _load_processed_dataset(
    pickle_path, size, preproc, batch_preproc, redo_preproc, verbose=False
):
    """Load and return the processed dataset, processing more data if necessary.

    Args:
        pickle_path (str): path to the target pickle file. Should be dataset_dir plus
                           the set name (e.g. 'train') plus the preprocessor name.
        size (int): number of samples to get.
        preproc (function): preprocessing function to apply to data.
        batch_preproc (bool): if True, preproc is called on the entire dataset at once.
                              Otherwise, preproc is called on a single data point and
                              label at a time.
        redo_preproc (bool): if True, preproc is called on the dataset whether or not a
                             cached, preprocessed version has already been created.
        verbose (bool): print debugging statements to stdout. Useful for development.
    Returns:
    ...
    if (
        not pickle_path.endswith("none.pkl")
        and os.path.isfile(pickle_path)
        and not redo_preproc
    ):
        ...
        utils.v_print(
            verbose, "    (cache size insufficient ({} < {}))".format(len(X), size)
        )
    else:
        # there wasn't enough preprocessed data in the cache (or we aren't getting
        # preprocessed data) so create empty containers for when we get the rest
        utils.v_print(
            verbose
            and (pickle_path.endswith("none.pkl") or not os.path.isfile(pickle_path)),
    ...
    unprocessed_path = os.path.join(
        os.path.dirname(pickle_path), "none.pkl"
    )  # location of unprocessed dataset
    ...
    # X/y should be the beginning of unproc_X/unproc_y, so only preprocess the end
    utils.v_print(
        verbose and X, "Cutting first {} from the unprocessed dataset".format(len(X))
    )
    utils.v_print(
        verbose and X,
        "    (len cut from {} to {})".format(len(unproc_X), len(unproc_X) - len(X)),
    )
    ...
    # apply preprocessor
    unproc_X, unproc_y = _apply_preproc(
        preproc, batch_preproc, unproc_X, unproc_y, verbose
    )

    # add the newly preprocessed data to whatever we had before
    utils.v_print(
        verbose, "Adding {} newly-preprocessed samples to set.".format(len(unproc_X))
    )
    X.extend(unproc_X)
    y.extend(unproc_y)
    ...
    if not pickle_path.endswith("none.pkl"):
        utils.v_print(
            verbose,
            'Writing resulting preprocessed samples to cache: "{}".'.format(
                pickle_path
            ),
        )
...
def get_data(
    ...
    """Return the train, val, or test set from the specified dataset, preprocessed as
    requested.

    If batch_preproc is set, the preprocessor must accept a list of data points and a
    list of corresponding labels. Otherwise, it must accept a single data point and its
    corresponding label. In either case, it should return preprocessed versions of both
    data and label.

    Args:
        dataset (str): name of dataset to load. Must be the name of a function defined
                       in datasets.py.
        subset (str): specifies the requested dataset; one of {'train', 'val',
                      'train+val', 'test', 'toy'}.
        size (int or NoneType): number of samples to include. If None, returns the
                                entire set of generated data.
        params (dict||utils.DefaultParamObject): parameters passed to data generation
                                                 function. If params is a dict, it will
                                                 be converted to the param class
                                                 corresponding to the dataset.
        preproc (callable): preprocessing function to be applied to data. If
                            batch_preproc is False, call signature must allow (x, y)
                            where x is a single data point and y is a label. If
                            batch_preproc is True, preproc is called on X, y where X is
    ...
    Returns:
        ...
        (list): labels corresponding to the data points.
    Raises:
        InsufficientSamplesError: if there are insufficient samples to create the
                                  requested dataset.
    """
    ...
    if subset.lower() == "test":
        warnings.warn(
            "getting test dataset; do not use except for producing final results"
        )
    elif subset.lower() == "train+val":
        # use get_data twice, and combine output. If either call to get_data ends up
        # insufficient, raise the sum as an error
        needed_samples = size
        try:
            X, y = get_data(
                dataset, "train", needed_samples, params, preproc, batch_preproc
            )
        except utils.InsufficientSamplesError as e:
            needed_samples = e.needed
            X, y = get_data(
                dataset, "train", size - needed_samples, params, preproc, batch_preproc
            )
        X2, y2 = get_data(
            dataset, "val", needed_samples, params, preproc, batch_preproc
        )
    ...
    return _load_processed_dataset(
        pickle_path, size, preproc, batch_preproc, redo_preproc, verbose
    )

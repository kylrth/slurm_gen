...
def _move(p):
    ...
    if p.from_set == "test":
        raise ValueError(
            'once samples have been added to "test", they cannot be removed'
        )
    ...
    print(
        "Move {} samples from {} to {}?".format(p.n_samples, from_dir_nice, to_dir_nice)
    )
    print(
        "This would result in sizes {} and {}, respectively.".format(
            from_count - p.n_samples, to_count + p.n_samples
        ),
        end="",
    )
    confirm = input(" (y/N): ")
    ...
    utils.to_pickle(
        (from_X[p.n_samples :], from_y[p.n_samples :]), os.path.join(from_dir, "none")
    )
    ...
    if p.from_set.lower() != "raw":
        for preproc_set in os.listdir(from_dir):
            if preproc_set != "none":
                ...
                utils.to_pickle(
                    (
                        from_X[min(p.n_samples, len(from_X)) :],
                        from_y[min(p.n_samples, len(from_y)) :],
                    ),
                    preproc_path,
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "dataset", nargs="?", help="Dataset to perform the list or move command on."
    )
    parser.add_argument(
        "-p",
        "--param_num",
    ...
    parser.add_argument(
        "-f", "--from_set", help="Subset of the dataset from which to move samples."
    )
    parser.add_argument(
        "-t", "--to_set", help="Subset of the dataset to which to move samples."
    )
    parser.add_argument(
        "-n", "--n_samples", type=int, help="Number of samples to move."
    )
    parser.add_argument(
        "-v",
        "--verbose",
